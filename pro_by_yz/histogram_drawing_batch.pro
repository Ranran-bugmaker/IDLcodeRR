pro histogram_drawing_batch
  ...
  subset_directory='O:/coarse_data/GMM/chapter_2/subset/';裁剪结果输出路径
  output_directory='O:/coarse_data/GMM/chapter_2/subset/png/';直方图结果输出路径
  shp_file='O:/coarse_data/GMM/chapter_2/sichuan_county_wgs84.shp';裁剪用矢量文件名
  ...
  for file_i=0,file_n-1 do begin;循环裁剪和直方图制图
    subset_file=subset_directory+file_basename(file_list[file_i],'.tiff')+'_subset.tiff'
    bar_file=output_directory+file_basename(file_list[file_i],'.tiff')+'_subset.png'
    ...
    bar=barplot(x,data_hist_freq,/histogram,xrange=[hist_xmin,hist_xmax],yrange=[hist_ymin,hist_ymax],xtickinterval=5,$
